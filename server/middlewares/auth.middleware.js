...
exports.isAdmin = async (req, res, next) => {
  ...
  const validate = mapping({ adminEmail }, userValidate.isAdminValSchema);
  if (validate.valid)
...
exports.isAdminWithToken = async (req, res, next) => {
  const token =
    req.body.token || req.query.token || req.headers["x-access-token"];
  ...
  const isAdminToken = await tokenService.isAdminToken(token);
  if (!isAdminToken)
    return res.status(403).send({ message: "Require Admin Role!" });
